...
class DistributionBuilder(object):
    ...
    def __load_from_yaml(self, yaml_file : str):
        ...
        if sum(self.config_cpu.values()) > 1: raise ValueError("CPU distribution frequency sum must be equal to one ")
        if sum(self.config_mem.values()) > 1: raise ValueError("Mem distribution frequency sum must be equal to one ")
