...
if (( "$#" != "1" ))
then
  echo "treat-dsb.sh Missing argument : ./treat-dsb.sh folder"
  exit -1
fi
folder="$1"
header="index,vm,timestamp,50th,70th,80th,90th,95th,99th,unreach,reqs,timeout,timeoutratio"
echo "$header" > output-dsb.txt
index=0
for file in $(ls $folder/*-dsb.txt); do
  index=$((index+1))
  ...
  echo "$index,$xvm,$xtmp,$xmed,$x70,$x80,$x90,$x95,$x99,$unreach,$reqs,$timeout,$timeoutratio"  >> output-dsb.txt
done
